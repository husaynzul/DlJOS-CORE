...
const PLATFORM_GROUPS = [
  {
    ...
    label: "Social Media",
    platforms: [
      { id: "instagram", name: "Instagram",   desc: "Posts, Reels, Stories",         authType: "oauth",  envKey: "META_CLIENT_ID",     oauthUrl: "https://developers.facebook.com/apps" },
      { id: "tiktok",    name: "TikTok",       desc: "Videos, TikTok LIVE",            authType: "oauth",  envKey: "TIKTOK_CLIENT_KEY",  oauthUrl: "https://developers.tiktok.com" },
      { id: "youtube",   name: "YouTube",       desc: "Videos, Shorts, Streams",        authType: "oauth",  envKey: "GOOGLE_CLIENT_ID",   oauthUrl: "https://console.cloud.google.com" },
      { id: "facebook",  name: "Facebook",      desc: "Pages, Groups, Feed",            authType: "oauth",  envKey: "META_CLIENT_ID",     oauthUrl: "https://developers.facebook.com/apps" },
      { id: "whatsapp",  name: "WhatsApp",      desc: "Business Messaging",             authType: "manual", envKey: "WHATSAPP_API_KEY",   manualUrl: "https://business.whatsapp.com/products/business-platform" },
      { id: "x",         name: "X (Twitter)",   desc: "Tweets, Threads",                authType: "manual", envKey: "X_API_KEY",           manualUrl: "https://developer.twitter.com/en/portal/dashboard" },
    ],
  },
  {
    id: "google",
    label: "Google System",
    platforms: [
      { id: "gmail",    name: "Gmail",         desc: "Send, read & automate email",    authType: "oauth",  envKey: "GOOGLE_CLIENT_ID",   oauthUrl: "https://console.cloud.google.com" },
      { id: "gdrive",   name: "Google Drive",  desc: "Files, Docs, Sheets",            authType: "oauth",  envKey: "GOOGLE_CLIENT_ID",   oauthUrl: "https://console.cloud.google.com" },
    ],
  },
  {
    id: "ads",
    label: "Ads System",
    platforms: [
      { id: "meta-ads",   name: "Meta Ads",   desc: "Facebook + Instagram Ads",       authType: "oauth",  envKey: "META_CLIENT_ID",     oauthUrl: "https://developers.facebook.com/apps" },
      { id: "google-ads", name: "Google Ads", desc: "Search, Display, YouTube Ads",   authType: "oauth",  envKey: "GOOGLE_CLIENT_ID",   oauthUrl: "https://console.cloud.google.com" },
      { id: "tiktok-ads", name: "TikTok Ads", desc: "TikTok Ad campaigns",            authType: "oauth",  envKey: "TIKTOK_CLIENT_KEY",  oauthUrl: "https://developers.tiktok.com" },
    ],
  },
];
...
function ConnectModal({
  ...
  const handleOAuth = () => {
    setLoading(true);
    setTimeout(() => { setLoading(false); onConnected(accountName.trim() || platform.name); }, 1200);
  };
      ...
      <div className="bg-background rounded-t-3xl max-h-[88dvh] overflow-y-auto animate-in slide-in-from-bottom duration-200" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-border">
          <div className="flex items-center gap-3">
            <BrandIcon platform={platform.name} size={24} withBackground />
            <div>
          ...
          {platform.authType === "oauth" && (
            <div className="space-y-3">
              ...
              <div>
                ...
                <input value={accountName} onChange={(e) => setAccountName(e.target.value)} placeholder="e.g. @myaccount"
              ...
              <div className="text-center"><span className="text-[12px] text-muted-foreground">or use API key instead ↓</span></div>
            </div>
          )}
          ...
          <div className="space-y-3">
            {platform.authType === "manual" && (
              <div className="space-y-2">
                ...
                {["Go to the developer console", "Create or copy your API key / token", "Paste it below to connect"].map((step, i) => (
                ...
                {(platform as any).manualUrl && (
                  <a href={(platform as any).manualUrl} target="_blank" rel="noreferrer"
            ...
            {(platform.authType === "manual" || apiKey.trim()) && (
              <button onClick={handleSave} disabled={!apiKey.trim() || loading}
                className={cn("w-full py-3.5 rounded-2xl text-[14px] font-semibold transition-all flex items-center justify-center gap-2",
...
export default function PlatformsPage() {
  ...
  const connect = (platform: PlatformDef, accountName: string) => {
    ...
    setStates(next); saveStates(next); setConnecting(null);
    ...
    <div className="flex flex-col h-full overflow-y-auto bg-background">
      ...
      <div className="px-4 py-4 space-y-5 max-w-[600px] mx-auto w-full pb-10">
        {PLATFORM_GROUPS.map((group) => (
          <div key={group.id}>
            <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-2 px-1">{group.label}</p>
              ...
              {group.platforms.map((platform) => {
                ...
                return (
                  <div key={platform.id} className="flex items-center gap-3.5 px-4 py-3.5">
                    {/* Brand icon */}
                    <div className="w-9 h-9 rounded-xl bg-muted/60 flex items-center justify-center flex-shrink-0 overflow-hidden">
                      <BrandIcon platform={platform.name} size={22} />
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className={cn("text-[10px] font-semibold px-1.5 py-0.5 rounded-md",
                          platform.authType === "oauth" ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground")}>
                    ...
                    {isConnected ? (
                      ...
                      <button onClick={() => setConnecting(platform)}
      ...
      {connecting && (
        <ConnectModal platform={connecting} onClose={() => setConnecting(null)} onConnected={(name) => connect(connecting, name)} />
      )}
    </div>
  );
}
